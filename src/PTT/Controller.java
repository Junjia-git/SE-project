...
//		}
	...
	public void loginManagement(String filePath) throws IOException {
		...
		else if (userInputC == 2) {
			teacherListProducing(filePath);
		}
		else if (userInputC == 3) {
			passToAdmin(filePath);
		}
		else if (userInputC == 4) {
			
		}
		else if (userInputC == 5) {
	...
	public void passToAdmin(String filePath) throws IOException {
		ma.readTeachingRequests();
		while(true) {
			vc.administratorManager();
			int userInputA = vu.userSelectionListener();
			switch(userInputA) {
				case 1:
					ma.findAllTeachingRequests();
					break;
				case 2:
					ma.selectLecturers();
					break;
...
//
//		default:
//			throw new IllegalArgumentException("Unexpected value: " + userInputB);
//		}
